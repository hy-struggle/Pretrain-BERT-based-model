...
parser.add_argument("--remove_nsp", action="store_true",
                    help="是否移除随机NSP任务负样本")
parser.add_argument("--do_ngram", action="store_true",
                    help="是否生成ngram数据，用于ZEN模型")
...
def get_wwm_tokens(segment, cws_set, word_length=args.max_word_len):
    """输入一句话，返回一句经过处理的话
    ...
    while i < len(segment):
        ...
        # token不是数字，直接加入
        if len(re.findall('^[0-9]*$', segment[i])) == 0:
        ...
        for length in range(word_length, 0, -1):
            if i + length > len(segment):
                continue
            if '-'.join(segment[i:i + length]) in cws_set:
                wwm_segment.append(segment[i])
...
def create_instances_from_document(
        ...
        masked_lm_prob, max_predictions_per_seq, whole_word_mask, vocab_list, ngram_dict=None):
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                if whole_word_mask:
                    # get cws set
                    cws_set = ['-'.join(tuple_word) for tuple_word in ngram_dict.id_to_ngram_list
                               if isinstance(tuple_word, tuple)]
                    tokens = get_wwm_tokens(tokens, cws_set=cws_set)
                ...
                if args.do_ngram:
                    ...
                    for p in range(2, args.max_word_len + 1):
        ...
        for epoch in trange(args.epochs_to_generate, desc="Epoch"):
            ...
            with open(metrics_file, 'w', encoding='utf-8') as metrics_file:
                metrics = {
                    ...
                    "max_ngram_in_sequence": ngram_dict.max_ngram_in_seq
                }
